with student as (
select * 
from {{ ref('input__student')}}
),

with course as (
select * 
from {{ ref('input__course')}}
),

with student_course_admission as (
select * 
from {{ ref('input__student_course_admission')}}
),
...
inner join student
on student.id = student_course_admission.student_id
inner join course
on course.id = student_course_admission.course_id
